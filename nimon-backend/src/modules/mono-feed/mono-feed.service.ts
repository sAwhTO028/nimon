...
export class MonoFeedService {
  ...
  private mapRowToSummary(
    ...
    const { coverUrl, publishKind, hasAudio } = this.extractContentMeta(row.content);
    ...
    return {
      ...
      writerAvatarUrl: row.owner?.profile?.avatarUrl ?? null,
      ...
      shareUrl: this.shareUrlFor(row.id),
      publishKind,
      accessType: 'public',
    };
  }
  ...
  async getPublicMonoById(
    ...
    const core = attachWriterProfileToDetail(publishedMonoDetailFromRow(m), writer);
