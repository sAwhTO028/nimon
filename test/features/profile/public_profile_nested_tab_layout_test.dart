...
class _FakeNestedLayoutPublicRepo extends RemotePublicCreatorProfileRepository {
  _FakeNestedLayoutPublicRepo({
...
void main() {
  ...
  testWidgets('M14J: public profile NestedScrollView + TabBar + list keys',
      (tester) async {
    ...
    expect(
        find.byKey(const ValueKey('publicProfileMonosList')), findsOneWidget);
  });

  testWidgets('M14J: toolbar username fades in when monos list scrolls',
      (tester) async {
    ...
    final before = _animatedOpacityAbove(titleEl);
    expect(before, isNotNull);
    expect(before!.opacity, lessThan(0.05));
    ...
    final after = _animatedOpacityAbove(
      tester
          .element(find.byKey(const ValueKey('publicProfileToolbarUsername'))),
    );
    expect(after, isNotNull);
    expect(after!.opacity, greaterThan(0.85));
  });

  testWidgets('M14J: monos scroll keeps TabBar + nested scaffold on screen',
      (tester) async {
    await tester.binding.setSurfaceSize(const Size(800, 1600));
    addTearDown(() => tester.binding.setSurfaceSize(null));
    ...
    final tabBarRect =
        tester.getRect(find.byKey(const ValueKey('publicProfileTabBar')));
    expect(tabBarRect.top, greaterThanOrEqualTo(0));
    expect(tabBarRect.bottom, lessThanOrEqualTo(1600));
  });

  testWidgets('M14J: Collections tab shows list key then Monos restores',
      (tester) async {
    final router = GoRouter(
      initialLocation: '/profile/public?userId=u1',
      routes: [
    ...
    expect(find.byKey(const ValueKey('publicProfileCollectionsList')),
        findsOneWidget);
  ...
  testWidgets('M14J: dark theme TabBar labels visible', (tester) async {
      ...
      ProviderScope(
        overrides: [
          remotePublicCreatorProfileRepositoryProvider.overrideWithValue(
            _FakeNestedLayoutPublicRepo(
              profile: profile,
              monoPage: PageResult<MonoFeedSummaryDto>(
                items: const [],
                nextCursor: null,
                hasMore: false,
                totalCount: null,
